...
@interface XLDrawerViewController () <UIGestureRecognizerDelegate>
...
@property (nonatomic, strong) NSHashTable<id<XLDrawerViewControllerDelegate>> *delegates;
@end
...
- (void)moveViewWithX:(CGFloat)x{
    ...
    self.menuViewController.view.frame = CGRectMake(kScreenshotImageOriginalLeft + y, 0, CGRectGetWidth(rect), CGRectGetHeight(rect));
}

#pragma mark -- XLDrawerViewController Methods
...
#pragma mark --Delgates Methods
- (void)bind:(id<XLDrawerViewControllerDelegate>)delegate {
    if (!delegate) {
        return;
    }
    
    if (![delegate conformsToProtocol:@protocol(XLDrawerViewControllerDelegate)]) {
        return;
    }
...
- (void)sendMenuDidAppearNotification{
    for (id<XLDrawerViewControllerDelegate> delegate in [self.delegates setRepresentation]) {
...
- (NSHashTable<id<XLDrawerViewControllerDelegate>> *)delegates {
    if (!_delegates) {
